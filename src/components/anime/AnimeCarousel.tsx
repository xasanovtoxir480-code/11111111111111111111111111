...
export default function AnimeCarousel({ anime }: AnimeCarouselProps) {
  const { navigate, setSelectedAnime } = useAppStore()
    ...
    <div className="relative overflow-hidden rounded-2xl">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="relative aspect-[16/7] w-full cursor-pointer overflow-hidden rounded-2xl md:aspect-[21/9]"
        >
          <img
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8">
            <div className="flex flex-wrap gap-2 mb-2">
              {anime[currentIndex]?.genres.split(',').map((genre) => (
                <span
                  key={genre}
